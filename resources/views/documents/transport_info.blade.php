		...
		<div class="form-row border-bottom pb-1">
			...
			<div class="form-group col-6">
				...
				<select name="tags[p18t1]" class="custom-select">
					...
					<option value="1" {{ $tags['p18t1'] == 1 ? 'selected' : '' }}>бензин</option>
				</select>
			</div>
		</div>
		<div class="form-row border-bottom pb-1">
		...
		<div class="form-row border-bottom">
	      ...
	      <div v-for="(row, i) in trailer_nums" class="input-group">
	      	...
	      	<div v-if="row != null" class="input-group-append">
	      		<button @click="removeInput(i)" class="btn btn-link input-group-text" type="button"><i class="fas fa-minus fa-xs"></i></button>
	      	</div>
	      </div>
			</div>
			<div class="form-group col-6">
